...
<head>
    <meta charset="UTF-8">
    <title>City Government of Malaybalay - CDR Report</title>
    <style>
        @page {
            margin: 0.5in 0.5in;
        }
...
<body>
    ...
    <div class="header">
        ...
        <div class="header-text">
            ...
            @endif
            @if(isset($filters['cheque_number']) && $filters['cheque_number'] != '')
                <p>Check No: {{ $filters['cheque_number'] }}</p>
            @endif
        </div>
    ...
    <table>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td class="text-left">{{ $item->nature_of_payment ?? '' }}</td>
                <td class="text-left">{{ $item->specific_fund ? \Illuminate\Support\Str::title($item->specific_fund) : '-' }}</td>
                <td class="text-right">{{ number_format($item->amount, 2) }}</td>
                <td class="text-center">{{ \Carbon\Carbon::parse($item->deposit_date)->format('d-M-y') }}</td>
            </tr>
            @endforeach

            @php $totalAmount = $data->sum('amount'); @endphp
            <tr class="totals-row">
