        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="space-y-2">
                ...
                <div class="flex flex-col md:flex-row md:items-center justify-between gap-1">
                    <div>
                        <h1 class="text-3xl font-black text-slate-950 tracking-tight">Add Deposit</h1>
                        <p class="text-slate-500 font-semibold text-sm">Create a new deposit record</p>
                    </div>
                </div>

                <!-- Form Box -->
                <div class="relative z-10 bg-white rounded-3xl p-4 border border-slate-200/60 shadow-xl shadow-slate-200/30">
                    <form method="POST" action="{{ route('deposits.store') }}" class="space-y-3">
                        @csrf
                            ...
                            <div class="space-y-2">
                                <div>
                                    ...
                                    <input type="number" step="any" name="amount" id="amount" value="{{ old('amount') }}" 
                                        class="relative z-50 block w-full rounded-xl border border-slate-200 bg-slate-50 text-slate-900 p-2 focus:ring-2 focus:ring-cyan-500 focus:ring-offset-2 focus:bg-white focus:border-transparent outline-none transition hover:bg-slate-100 !pointer-events-auto cursor-text" required>
                                    @error('amount')
                            ...
                            <div class="space-y-2">
                                <div>
                                    ...
                                    <textarea name="nature_of_payment" id="nature_of_payment" rows="1" 
                                        class="w-full rounded-xl border border-slate-200 bg-slate-50 text-slate-900 p-2 focus:ring-2 focus:ring-cyan-500 focus:ring-offset-2 focus:bg-white focus:border-transparent outline-none transition hover:bg-slate-100">{{ old('nature_of_payment') }}</textarea>
                                ...
                                <div>
                                    <label for="deposit_date" class="block text-sm font-black text-slate-600 uppercase tracking-wider mb-2">Date</label>
                                    <input type="date" name="deposit_date" id="deposit_date" value="{{ old('deposit_date', date('Y-m-d')) }}" 
                        ...
                        <div class="flex items-center justify-center mt-4 pt-2 border-t border-slate-200">
                            <a href="{{ route('deposits.index') }}" class="mr-4 px-6 py-3 text-slate-600 font-bold hover:text-slate-900 border border-slate-200 rounded-xl hover:bg-slate-50 transition-all duration-300 cursor-pointer">
                                Cancel
                            </a>
