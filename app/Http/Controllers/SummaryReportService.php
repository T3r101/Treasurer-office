...
class SummaryReportService
{
    public function getSummaryReport(array $filters = []): array
    {
        ...
        return Cache::remember($cacheKey, now()->addMinute(), function () use ($startDate, $endDate) {
    ...
    {
        $query = Transaction::withoutGlobalScopes()->whereIn('type', ['expense', 'Expense', 'EXPENSE']) // Removed user_id filter
            ->where('status', 'active');
    ...
    {
        $query = Deposit::withoutGlobalScopes()->where('status', 'active'); // Removed user_id filter
    ...
    {
        return (float) Transaction::withoutGlobalScopes()->where('type', 'income') // Removed user_id filter
                                ->where('status', 'active')
    ...
    {
        return (float) Transaction::withoutGlobalScopes()->where('type', 'expense') // Removed user_id filter
                                ->where('status', 'active')
                                ->whereBetween('transaction_date', [$start, $end])
                                ->sum(\DB::raw('COALESCE(NULLIF(amount_issued, 0), amount)'));
    }
    ...
    {
        return (float) Deposit::withoutGlobalScopes()->where('status', 'active') // Removed user_id filter
    ...
    {
        $currentTotal = Transaction::withoutGlobalScopes()->where('status', 'active') // Removed user_id filter
                                    ->where('type', 'income')
                                    ->where('transaction_date', '>=', now()->startOfMonth())
                                    ->sum(\DB::raw('COALESCE(NULLIF(amount_issued, 0), amount)'));
        $previousTotal = Transaction::withoutGlobalScopes()->where('status', 'active') // Removed user_id filter
                                    ->where('type', 'income')
